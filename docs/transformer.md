...
- The type walk memoizes a named object type's resolved `Field` by its
  `ts.Symbol` so a self-referential type only gets walked once. That cache
  must also be keyed by whether the walk was `packed` — the same
  `interface` can legitimately appear twice in one root type, once as a
  plain field and once inside a `Packed<T>` subtree (e.g. a shared
  `Flags` interface reused as both `plain: Flags` and
  `wrapped: DataType.Packed<Flags>`), and those two walks must produce
  different `Field`s (only one has its booleans bit-packed). Keying by
  symbol alone silently reused whichever variant was walked first for
  both. Fixed by keying the cache on `(symbol, packed)`; pinned by a
  dedicated case in the transformer repo's `test/walk.test.ts`. One narrower
  residual case is still open, not yet fixed: a **self-referential**
  type reused in both a plain and a `Packed<T>` context still generates
  only one recursion helper (keyed by symbol alone, not `(symbol,
packed)`), so the second context's recursive references would resolve
  through the first context's helper. Realistically this needs a
  recursive type to be deliberately reused both plain and packed inside
  the same root shape — narrow enough that it's called out here rather
  than fixed, but a real gap if hit.
...
| --------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `f32`/`f64`/`u8`/`u16`/`u32`/`i8`/`i16`/`i32`       | `DataType.f32` etc. branded numbers (`number & { _u8?: never }`), or plain `number` → `f64` by default                      | `num(width)`                                                | direct `buffer.writeXX`/`readXX`, no branching                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `cframe`                                            | `CFrame`                                                                                                                    | `cframe`                                                    | 6×`f32` (position + axis-angle) by default; axis-aligned/zero-vector size optimization only inside `Packed<T>`, matching fbs                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `color3`                                            | `Color3`                                                                                                                    | `color3`                                                    | 3×`u8` (0–255 channel quantization, same precision trade-off fbs makes)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `colorsequence` / `numbersequence`                  | `ColorSequence` / `NumberSequence`                                                                                          | `colorSequence` / `numberSequence`                          | `u8` keypoint count + fixed-size keypoints, same layout fbs uses                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `enum`                                              | `EnumItem`                                                                                                                  | `enum(enumName)`                                            | `u8` index into a **compile-time-computed** `{[EnumItem]: index}` lookup table emitted as a module constant, entries in **member-name-sorted order** (same discipline as object field sorting, Transformer Design §3 — not `.d.ts` declaration order or a runtime `GetEnumItems()` call, neither of which this design relies on being stable) — O(1), unlike fbs's runtime `indexOf` scan                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `optional`                                          | `T \| undefined`                                                                                                            | `optional(inner)`                                           | 1-byte presence flag by default; bit-packed only inside `Packed<T>`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `literal` (multi-value)                             | literal union (`"a" \| "b" \| "c"`, numeric/boolean literal unions, including one that also admits `undefined`)             | `literal(values[])`                                         | `u8`/`u16` index into a fixed values table, chosen by value count, matching fbs's width thresholds                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `union` (discriminated)                             | object union sharing one unique-per-variant literal key                                                                     | `taggedUnion(tagKey, variants[])`                           | `u8`/`u16` variant index + inline variant fields (tag key omitted from the payload, restored on deserialize)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `union` (guard, ≤1 table-shaped variant)            | e.g. `string \| number`, `SomeInterface \| string`                                                                          | `guardedUnion(variants[])`                                  | 1-byte variant index; **this design** emits a direct `typeIs(value, "string")`/`"number"`/`"boolean"` check per primitive constituent instead of a generated guard — simple because these are Luau built-in type tags, not because fbs does the same. fbs, by contrast, still requests a Flamework-generated guard (`Modding.Generic<T, "guard">`) for every non-table constituent; only the one lucky "singular object constituent" gets `undefined` in place of a guard, falling back to `typeIs(value, "table")`. **"Table-shaped" is deliberately broader than "object":** it must count objects, arrays, tuples, and `dict` (`Map`/`Set`/`Record`) constituents together, since all of them compile to a plain Luau table and are equally indistinguishable via `typeIs(value, "table")` — fbs's own `IsTableObject<T>` check is written this way for the identical reason (`T extends object` is true for arrays too, so fbs already counts an array constituent against the "≤1" budget, not just plain interfaces) |
...
via a branded wrapper type: `DataType.Packed<T> = T & { _packed?: [T] }`.
Only `boolean`, `optional`, and `cframe` fields _inside_ a `Packed<T>`
subtree get bit-packed (booleans/optionals collapse to 1 bit each, written
once as trailing packed bytes; `CFrame` gets the axis-aligned-rotation /
zero-vector size optimization); everywhere else these three kinds are
byte-aligned. This design adopts the same opt-in mechanism rather than
...
"guard">`), which a standalone transformer has no access to and would
  ...
  transformer throws a build-time error naming the union and telling the
  ...
  unions).
- **`Packed<T>` bit-packs `boolean` fields only — not `optional` or
  `cframe`.** Implementing packed `optional` correctly needs the
  presence bit and the inner payload to agree on read/write order even
  though the payload's byte position and the bit's trailing position are
  computed independently; implementing packed `cframe` needs choosing and
  documenting an exact axis-aligned/zero-vector byte layout, which fbs's
  own source would need to be read line-by-line to match rather than
  invented. Both are real, addressable gaps, not fundamental blockers —
  just not done yet. Packed `boolean` has none of this ambiguity (the bit
  fully represents the value, so its position within the trailing region
  is the only thing that has to agree between write and read) and is
  fully implemented, per-object (each object literal gets its own
  trailing packed-bit region for its own direct packed `boolean`
  properties; a packed `boolean` reached through an array or dict element
  falls back to the byte-aligned encoding, since the packed region's size
  has to be known at compile time and an array's length isn't).
- **Luau function-size limits: not yet implemented, still a real risk for
  pathologically large single structs.** 200 locals / 200 upvalues per
  function (Luau `Compiler.cpp`, `luau.org/compatibility`) count only
  _declared_ variables, not statement count; a flat sequence of hundreds
  of `buffer.writeXX` calls with no local declarations does not approach
  either limit. The real ceiling is the instruction-count limit (2^23
  interpreted; a tighter 64K-instructions-per-block applies only under
  `@native`). Only pathologically large single structs (hundreds of
  fields) would need scope-splitting into helper functions — this design
  does not yet detect that case or split scope automatically; a struct
  large enough to hit either limit would currently fail to compile (or
  compile a function Luau itself refuses to run) rather than being
  handled gracefully.
