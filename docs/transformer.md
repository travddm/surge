...
- **`Packed<T>` bit-packs `boolean` and `optional` properties — not a
  tagged-union tag.** A 2-way tagged-union tag still costs a byte (fbs
  packs it into a bit). A real, addressable gap, not a fundamental
  blocker — just not done yet. Packing is per object: each object gets its own
