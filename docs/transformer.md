...
adjacent bits (presence, then value) and has no byte of its own. The
region comes first because the read side needs a presence bit before it
reaches the optional's value. The packed `CFrame` form (the
axis-aligned-rotation / zero-vector size optimization) is not implemented;
see Risks. This design adopts the same opt-in mechanism rather than
...
"guard">`), which a standalone transformer has no access to and would
  ...
  unions).
- **`Packed<T>` bit-packs `boolean` and `optional` properties — not a
  tagged-union tag or a `cframe`.** A 2-way tagged-union tag still costs a
  byte (fbs packs it into a bit). Packed `cframe` needs choosing and
  ...
  just not done yet. Packing is per object: each object gets its own
  leading packed region for its own direct packed `boolean` and
  `optional` properties. A packed `boolean` or `optional` that is not a
  direct property of an object — an array, tuple, or dict element, a
  union variant that is not an object, or the root type — falls back to
  the byte-aligned encoding with no diagnostic, since the packed region's
  size has to be known at compile time and an array's length isn't.
