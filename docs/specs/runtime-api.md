...
type Serialized<T> = { buffer: buffer; blobs: Array<defined> } | { buffer: buffer; blobs?: undefined };
```

This is the shape and blob-array calling convention of
flamework-binary-serializer's `Serializer<T>`, except that the result has no
`blobs` array for a `T` that can hold no blob (3.6).
...
**3.6** `Serialized<T>` declares a `blobs` array when `T` can hold a blob
field, as 3.12 decides, and `blobs?: undefined` otherwise. Where it declares
the array, `serialize(value).blobs` holds the values the encoding passed
through the blob channel, in the order the encoding met them, and is empty
when there were none. Where it does not, `serialize` returns the buffer alone.
...
once. Where it cannot tell, it declares the array, which is then always empty.
A type that declares its own `_nominal_*` property is one such `T`. A call site
whose `serialize` passes a blob that `Serialized<T>` does not declare is a
diagnostic (Transformer 7.3).
