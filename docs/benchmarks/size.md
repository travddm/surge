...
buffer does not show: the blob array in surge, fbs, and serio, the instance
table in Zap, and nothing in Blink, whose exports cannot carry an Instance
or an unknown at all.
...
row. Blink and Zap are both smaller than surge wherever a length prefix
...
rebuilds a rotation by passing the unnormalized axis-angle vector to
`CFrame.fromAxisAngle`, which Lune does not normalize (surge and fbs pass
