...
| ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | [generated-code-performance.md](generated-code-performance.md): the per-call gap first, starting with what `serialize()` returns, which is a decision its measurement informs; then fewer reservations and exact sizing, each measured on its own; then the rest of the document | The per-call gap is the largest open item. Most of it is three tables per call, measured in [tables-around-serialize.md](../research/tables-around-serialize.md), and removing the two `serialize()` returns changes what a caller gets, which is free to change before the first release. Step 2 waits on it. |
