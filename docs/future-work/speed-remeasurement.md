...
| Blob side channel 0.997× and `finishWrite` copy 1.007×: per-call costs are worth nothing                                         | generated-code-performance.md, Blob side channel and finishWrite                                | Full runs; quiet cells in both halves                      | First: the current file shows a 0.2 µs per-call gap, see above |
